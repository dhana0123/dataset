...
"""Indic Inner Monologue dataset packer (standalone under ``data/``).

Writes the on-disk format consumed by ``moshi.train``:

* 24 kHz stereo WAV (left = agent / Moshi, right = user)
* sibling ``.json`` with word alignments on the **agent** channel only
* ``jsonl`` rows ``{"path", "duration", "extraction"?}``

Mimi RVQ and PAD/EPAD interleaving happen in the moshi train loader.
"""
...
logger = logging.getLogger("duplex_data.prepare")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Indic Inner Monologue packer (data/duplex_data) — dual-channel + agent ASR."
    )
    ...
    parser.add_argument(
        ...
        help="Agent ASR: Whisper (default) or IndicConformer+WhisperX (needs AI4Bharat NeMo).",
    )
    parser.add_argument(
        "--asr-model",
        default=None,
    ...
    parser.add_argument(
        ...
        default=True,
        help="Mono → pyannote community-1 gate to stereo (default: on).",
    )
    parser.add_argument(
        "--text-delay-frames",
        type=int,
    ...
    parser.add_argument("--device", default="cuda", help="Device for ASR / diarize.")
    parser.add_argument(
        "--push-to-hub",
        action=argparse.BooleanOptionalAction,
        default=True,
    ...
    parser.add_argument(
        "--hf-dataset",
        default=None,
        help="Dataset repo id or name (default: <you>/dupxel-indic).",
    )
